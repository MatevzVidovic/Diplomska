...
class IrisDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # print(sclera.shape)
            # print(veins.shape)
            # Both are 4d tensors now. [batch_size, channels, height, width]
            # input("Press Enter to continue...")
            # PyTorch needs img to be 4d, and target to be 3d. Respectively: [batch_size, channels, height, width] and [batch_size, height, width]
            # It has no channels. The different classes are represented as diferent uints in the same image.

            sclera_target = sclera
            sclera_target[sclera_target > 127] = 1
            sclera_target = sclera_target.squeeze(1)

            veins = veins.squeeze(1)

            multi_target = sclera_target
            multi_target[veins == 1] = 2

            # multi_target = smart_conversion(multi_target, 'tensor', "uint8").long() # converts to int64
            multi_target = multi_target.long() # converts to int64



            return {'images': img, 'multi': multi_target, "scleras": torch.zeros(size=[1]) , 'img_names': img_name_no_suffix}
...
def custom_collate_fn(batch):
    ...
    multi = torch.concat([item['multi'] for item in batch], dim=0)
    scleras = torch.concat([item['scleras'] for item in batch], dim=0)

    img_names = []
    for item in batch:
        img_names.extend(item['img_names']) 
    return {'images': images, 'multi': multi, "scleras" : scleras, 'img_names': img_names}
...
if __name__ == "__main__":
    ...
    handlers = py_log_always_on.file_handler_setup(MY_LOGGER, add_stdout_stream=False)
...
#    for i in range(1000):
    res = train_dataset[0]
    py_log_always_on.log_manual(MY_LOGGER, img=res['images'], multi=res['multi'], names=res['img_names'])
    for i in range(dataloading_args["num_of_patches_from_img"]):
        ...
        save_img_quick_figs(res['multi'][i]*127, f"test_{i}_multi.png")
