...
# - save
# - prune (2 prunings by 0.1 percent, between them 2 train iters)
# - save
# - prune (2 prunings by 0.1 percent, before each 2 train iters)
# - save
...
python3 segnet_main.py 0 4 10 1 SegNet ./sclera_data < "$save_and_stop"          2>> "${rfn}/curr/stderr_log_{cbi}.txt" 2>&1 | tee "${rfn}/curr/${obn}_${cbi}_${cn}.txt"; cn=$((cn + 1))

python3 segnet_main.py 999999 4 10 1 SegNet ./sclera_data --mti 2          2>> "${rfn}/curr/stderr_log_{cbi}.txt" 2>&1 | tee "${rfn}/curr/${obn}_${cbi}_${cn}.txt"; cn=$((cn + 1))
