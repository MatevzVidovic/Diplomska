...
python3 ${main_name} --bs ${bs} --nodw ${nodw} --sd ${folder_name} --ptd ${ptd} --lr ${lr} --tesl ${tesl} --iw ${iw} --ih ${ih} -m ${model_name} --ntibp ${ntibp} --pruning_phase --pbop --map 70 --rn flops_num --ptp 0.01 --ifn 1            2>&1 | tee "${rfn}/curr/${obn}_${cbi}_${cn}.txt"; cn=$((cn + 1))
