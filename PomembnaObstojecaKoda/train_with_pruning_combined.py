...
def _get_bn_by_prev_conv_index(conv_index, model):
    ...
    if 'densenet' in model_name:
        ...
        assert conv_index < 18
        if conv_index < 2:
            layer_name = f'inc.bn{conv_index + 1}'
        else:
            conv_index -= 2
            du = 'up' if conv_index // 8 else 'down'
            block = conv_index % 8 // 2 + 1
            layer = conv_index % 2 + 1
            layer_name = f'{du}{block}.conv.bn{layer}'
        return op.attrgetter(layer_name)(model)
...
def remove_filters_and_retrain_model(args, device, trainloader, testloader, start_epoch, end_epoch, viz, win_train_mIoU_during_retraining,
    ...
    while n_removed_flops_for_this_step < parameters['n_removed_flops_for_this_step']:
        ...
        #all_activations_sum_dict_sorted = _get_sorted_filter_activations_dict_for_model(model)
        all_importances_dict_sorted = _get_sorted_filter_importance_dict_for_model(model, parameters['omega'], args)

        # filter out 1x1 or 3x3 filters, depending on the passed arguments (densenet only - UNet only has 3x3 filters anyway)
        if args.prune != 'all':
            filter_re = re.compile(r'conv(1|.2)') if args.prune == 'conv' else re.compile(r'conv.1')
            all_importances_dict_sorted = {k: v for k, v in all_importances_dict_sorted.items() if filter_re.search(k)}
        ...
        key, value = get_filter_with_minimal_importance(device, all_importances_dict_sorted, layers_with_exceeded_limit, blocks_with_exceeded_limit)
...
def count_zeroed_filters_for_model(model, device):
    ...
    for name, param in model.named_parameters():  # always weight and bias
        if name.endswith('_activations_sum'):
            block_name, _, layer_name = name.replace('_activations_sum', '').rpartition('.')
...
def get_zeroed_and_all_parameters_wrt_layer_for_model_and_zeroed_percent_for_layer_and_block(model, device):
    ...
    for name, module in model.named_modules():  # always weight and bias
        ...
        if 'densenet' in model_name and len(name.split('.')) != 2:
            print('skipping {0}'.format(name))
            continue # do not count out_conv on model itself
        if 'unet' in model_name and 'outc' in name:
            print('skipping {0}'.format(name))
            continue # do not count out_conv on model itself
...
def start_pruning(args, device, trainloader, model, validloader, testloader, teacher_model, device_teacher, viz, parameters, criterion, criterion_DICE, criterion_SL):
    ...
    while True:
        ...
        # First 25% removed should be saved to 25, the next to 50, and the last to final
        curr_removed = 100 - 100 * rm.cur_flops / original_flops
        suffix = int(25 * np.ceil((curr_removed + 1) / 25)) if curr_removed <= parameters['prune_away_percent'] - 25 else 'final'
        torch.save(model, LOGDIR/'models'/f'{args.model}_{suffix}.pt')
...
def main():
    ...
    # Parse omega out of expname if possible
    try:
        omega = float(re.search(r'omega([0-9e\-\.]*)', args.expname).group(1))
    except (ValueError, AttributeError):
        logger.write("Could not parse ω from expname, using default value")
        omega = 1

    # Do the same with distillation parameters
    try:
        dalpha = float(re.search(r'alpha([0-9e\-\.]*)', args.expname).group(1))
    except (ValueError, AttributeError):
        logger.write("Could not parse α from expname, using default value")
        dalpha = 0
    try:
        dtemp = float(re.search(r'temp([0-9e\-\.]*)', args.expname).group(1))
    except (ValueError, AttributeError):
        logger.write("Could not parse τ from expname, using default value")
        dtemp = 8
    try:
        dbeta = float(re.search(r'beta([0-9e\-\.]*)', args.expname).group(1))
    except (ValueError, AttributeError):
        logger.write("Could not parse β from expname, using default value")
        dbeta = 0
    try:
        dlambda = float(re.search(r'lambda([0-9e\-\.]*)', args.expname).group(1))
    except (ValueError, AttributeError):
        logger.write("Could not parse λ from expname, using default value")
        dlambda = 0
    ...
    if 'unet' in args.model.lower():
        logger.write("UNet has ~10x as many total FLOPs so we remove 10x as many each iteration")
        parameters['n_removed_flops_for_this_step'] *= 10
