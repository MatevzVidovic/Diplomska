...
from model.ritnet import DenseNet2D_down_block_original, DenseNet2D_up_block_concat_original
...
class FilterPrunerRitnet(FilterPruner):
    def trace_layer(self, layer, x):
        ...
        if isinstance(layer, nn.Conv2d):
            ...
            if self.rank_type == 'l1_weight':
                ...
                pass
            elif 'combined' in self.rank_type:
                if self.activation_index not in self.filter_ranks:
                    self.filter_ranks[self.activation_index] = torch.zeros(layer.weight.size(0), device=self.device)
                weights = layer.weight.detach()
                activations = y.detach()
                diff_matrices = activations.mean(dim=0) - activations.mean(dim=(0, 1))
                if 'l1' in self.rank_type:
                    weight_component = weights.abs().sum(dim=(1, 2, 3))
                    activation_component = diff_matrices.abs().mean(dim=(1, 2))
                elif 'l2' in self.rank_type:
                    weight_component = weights.pow(2).sum(dim=(1, 2, 3)).sqrt()
                    activation_component = diff_matrices.pow(2).sum(dim=(1, 2)).sqrt() / (diff_matrices.shape[1] * diff_matrices.shape[2])
                ...
                self.filter_ranks[self.activation_index] = self.omega * weight_component + (1 - self.omega) * activation_equalizer * activation_component
            else:
    ...
    def parse_dependency(self):
        ...
        prev_res = -1
        for m in self.model.modules():
            if isinstance(m, DenseNet2D_down_block_original):
                cur_conv_idx = self.conv_to_index[m.conv32] # last convolution in block
                # if there is auxiliary 1x1 conv on shortcut
                # TODO this was removed because there is no conv on my shortcut only AvgPool
                #if isinstance(m.shortcut, DownsampleA):
                #    self.pre_padding[cur_conv_idx] = m.shortcut
                self.chains[prev_conv_idx] = cur_conv_idx
                prev_conv_idx = cur_conv_idx

            if isinstance(m, DenseNet2D_up_block_concat_original):
                cur_conv_idx = self.conv_to_index[m.conv22] # last convolution in block
                # if there is auxiliary 1x1 conv on shortcut
                # TODO this was removed because there is no conv on my shortcut only AvgPool
                #if isinstance(m.shortcut, DownsampleA):
                #    self.pre_padding[cur_conv_idx] = m.shortcut
        ...
        last_idx = -1
        for m in self.model.modules():
            if isinstance(m, nn.Conv2d) and m.weight.size(2) == 3: # TODO: ce je kernel 3x3?? -> to majo itak vsi..kaj pa pri meni??? moram dodati enko??
            ...
            if isinstance(m, nn.Conv2d) and m.weight.size(2) == 1:
    ...
    def prune_conv_layer_segment(self, layer_index, filter_range):
        #print('pruning layer ' + str(layer_index))
        filters_begin = filter_range[0]
        filters_end = filter_range[1]
        ...
        if conv.groups == conv.out_channels and conv.groups == conv.in_channels:
            print("===========================# Prunned filters: {}\nnew_conv in_channels: {}\nnew_conv out_channels: {}\nnew_conv groups: "
                  .format(pruned_filters, conv.out_channels - pruned_filters, conv.out_channels - pruned_filters, conv.groups - pruned_filters))
            ...
            new_conv = \
                torch.nn.Conv2d(in_channels=conv.out_channels - pruned_filters, \
                                ...
                                bias=True) #TODO original code: conv.bias
            ...
            #print("# Prunned filters: {}\nnew_conv in_channels: {}\nnew_conv out_channels: {}\nnew_conv groups: {}"
            #      .format(pruned_filters, conv.in_channels, conv.out_channels - pruned_filters,
            #              conv.groups))
            new_conv = \
        ...
        conv.weight = torch.nn.Parameter(torch.from_numpy(new_weights).to(self.device))
        conv.weight.grad = None
        ...
        if not conv.bias is None:
            ...
            conv.bias = torch.nn.Parameter(torch.from_numpy(bias).to(self.device))
            conv.bias.grad = None

        # prune next bn if nedded
        self._prune_next_bn_if_needed(layer_index, filters_begin, filters_end, pruned_filters)

        #surgery on chained convolution layers
        next_conv_idx_list = self._get_next_conv_id_list_recursive(layer_index)
        for next_conv_id in next_conv_idx_list:
            #print(next_conv_id)
            self._prune_next_layer(next_conv_id, filters_begin, filters_end, pruned_filters)


    def _get_next_conv_id_list_recursive(self, layer_index):
        # Find next convolution layers
        if layer_index == 0:
            next_conv_idx = [1, 3]
        elif layer_index == 1:
            next_conv_idx = [2]
    ...
    def _prune_next_bn_if_needed(self, layer_index, filters_begin, filters_end, pruned_filters):
        ...
        if next_bn_index is not None:
            next_bn = self.bn_for_conv[next_bn_index]
        ...
        if next_bn is not None:
            ...
            new_weights[: filters_begin] = old_weights[: filters_begin]
            new_weights[filters_begin:] = old_weights[filters_end + 1:]
            next_bn.weight = torch.nn.Parameter(torch.from_numpy(new_weights).to(self.device))
            next_bn.weight.grad = None

            new_bias[: filters_begin] = old_bias[: filters_begin]
            new_bias[filters_begin:] = old_bias[filters_end + 1:]
            next_bn.bias = torch.nn.Parameter(torch.from_numpy(new_bias).to(self.device))
            next_bn.bias.grad = None
    ...
    def _prune_next_layer(self, next_conv_i, filters_begin, filters_end, pruned_filters):
        ...
        next_conv.weight = torch.nn.Parameter(torch.from_numpy(new_weights).to(self.device))
