...
class DenseNet2D_down_block(nn.Module):
    def __init__(self, input_channels, output_channels, down_size, downsize_activations, activation_height,
    ...
    def forward(self, x, prev_x=None):
        ...
        if self.dropout:
            ...
            self.conv1_activations = self._set_activations_for_removed_filters_to_zero(self.conv1,
                                                                                       self.conv1_activations)
            x1 = self.relu(self.dropout1(self.conv1_activations))
            first_fsp_layer = x1
            x21 = torch.cat((x, x1), dim=1)
            self.conv21_activations = self.conv21(x21)
            self.conv21_activations = self._set_activations_for_removed_filters_to_zero(self.conv21,
                                                                                        self.conv21_activations)
            self.conv22_activations = self.conv22(self.conv21_activations)
            self.conv22_activations = self._set_activations_for_removed_filters_to_zero(self.conv22,
                                                                                        self.conv22_activations)
            x22 = self.relu(self.dropout2(self.conv22_activations))
            x31 = torch.cat((x21, x22), dim=1)
            self.conv31_activations = self.conv31(x31)
            self.conv31_activations = self._set_activations_for_removed_filters_to_zero(self.conv31,
                                                                                        self.conv31_activations)
            self.conv32_activations = self.conv32(self.conv31_activations)
            self.conv32_activations = self._set_activations_for_removed_filters_to_zero(self.conv32,
                                                                                        self.conv32_activations)
    ...
    def save_activations_and_sum(self, activations):
        ...
        if self.downsize_activations:
            activations = nn.functional.interpolate(activations, size=(self.activation_height, self.activation_width),
                                                    mode='bilinear',
                                                    align_corners=False)  # torch.nn.functional.interpolate(a, size=(4, 3), mode='bilinear', align_corners=False)
    ...
    def reset_conv_activations_sum(self, device=None):
        ...
        if device:
            self.conv1_activations_sum = torch.nn.Parameter(torch.zeros(self.output_channels).to(device),
                                                            requires_grad=False)
            self.conv21_activations_sum = torch.nn.Parameter(torch.zeros(self.output_channels).to(device),
                                                             requires_grad=False)
            self.conv22_activations_sum = torch.nn.Parameter(torch.zeros(self.output_channels).to(device),
                                                             requires_grad=False)
            self.conv31_activations_sum = torch.nn.Parameter(torch.zeros(self.output_channels).to(device),
                                                             requires_grad=False)
            self.conv32_activations_sum = torch.nn.Parameter(torch.zeros(self.output_channels).to(device),
                                                             requires_grad=False)
        else:
            self.conv1_activations_sum = torch.nn.Parameter(torch.zeros(self.output_channels),
                                                            requires_grad=False)  # 1, output_c, 640, 400
            self.conv21_activations_sum = torch.nn.Parameter(torch.zeros(self.output_channels),
                                                             requires_grad=False)  # 1, output_c, 640, 400
            self.conv22_activations_sum = torch.nn.Parameter(torch.zeros(self.output_channels),
                                                             requires_grad=False)  # 1, output_c, 640, 400
            self.conv31_activations_sum = torch.nn.Parameter(torch.zeros(self.output_channels),
                                                             requires_grad=False)  # 1, output_c, 640, 400
            self.conv32_activations_sum = torch.nn.Parameter(torch.zeros(self.output_channels),
                                                             requires_grad=False)  # 1, output_c, 640, 400


# modified original to have pruning capabilities
class DenseNet2D_up_block_concat(nn.Module):
    def __init__(self, skip_channels, input_channels, output_channels, up_stride, downsize_activations,
                 activation_height,
                 activation_width, activations_weighting_power, dropout=False, prob=0, ):
    ...
    def forward(self, prev_feature_map, x, residual_x=None):
        ...
        if self.dropout:
            ...
            self.conv11_activations = self._set_activations_for_removed_filters_to_zero(self.conv11,
                                                                                        self.conv11_activations)
            self.conv12_activations = self.conv12(self.conv11_activations)
            self.conv12_activations = self._set_activations_for_removed_filters_to_zero(self.conv12,
                                                                                        self.conv12_activations)
    ...
    def reset_conv_activations_sum(self, device=None):
        # print('resetting activations sum for all layers..')
        if device:
            self.conv11_activations_sum = torch.nn.Parameter(torch.zeros(self.output_channels).to(device),
                                                             requires_grad=False)
            self.conv12_activations_sum = torch.nn.Parameter(torch.zeros(self.output_channels).to(device),
                                                             requires_grad=False)
            self.conv21_activations_sum = torch.nn.Parameter(torch.zeros(self.output_channels).to(device),
                                                             requires_grad=False)
            self.conv22_activations_sum = torch.nn.Parameter(torch.zeros(self.output_channels).to(device),
            ...
            self.conv11_activations_sum = torch.nn.Parameter(torch.zeros(self.output_channels),
                                                             requires_grad=False)  # 1, output_c, 640, 400
            self.conv12_activations_sum = torch.nn.Parameter(torch.zeros(self.output_channels),
                                                             requires_grad=False)  # 1, output_c, 640, 400
            self.conv21_activations_sum = torch.nn.Parameter(torch.zeros(self.output_channels),
                                                             requires_grad=False)  # 1, output_c, 640, 400
            self.conv22_activations_sum = torch.nn.Parameter(torch.zeros(self.output_channels),
...
class DenseNet2D(nn.Module):
    def __init__(self, in_channels=1, out_channels=4, channel_size=32, downsize_activations=False, activation_height=40,
                 activation_width=25, activations_weighting_power=None, concat=True, dropout=False, prob=0):
        ...
        self.down_block1 = DenseNet2D_down_block(input_channels=in_channels, output_channels=channel_size,
                                                 ...
                                                 downsize_activations=downsize_activations,
                                                 activation_height=activation_height, activation_width=activation_width,
                                                 activations_weighting_power=activations_weighting_power)
        ...
        self.up_block1 = DenseNet2D_up_block_concat(skip_channels=channel_size, input_channels=channel_size,
                                                    ...
                                                    prob=prob, downsize_activations=downsize_activations,
